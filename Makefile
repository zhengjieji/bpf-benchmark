...
AWS_BASE_ENV = TARGET=$(AWS_TARGET_NAME.$(AWS_ARCH_KEY)) RUN_TARGET_NAME=$(AWS_TARGET_NAME.$(AWS_ARCH_KEY)) \
	...
	RUN_AMI_ID="$($(AWS_PREFIX.$(AWS_ARCH_KEY))_AMI_ID)" \
...
SUITE_ENV_NAMES = SAMPLES WARMUPS WARMUP_REPEAT INNER_REPEAT BENCH SUITE RUNTIMES FUZZ_ROUNDS TEST_MODE WORKLOAD_DURATION \
...
MICRO_IMAGE_TARGET.x86.full := x86-runner-runtime-image-tar
MICRO_IMAGE_TARGET.arm64.full := arm64-runner-runtime-image-tar
MICRO_IMAGE_TARGET.x86.characterization := x86-micro-characterization-image-tar
MICRO_IMAGE_TARGET.arm64.characterization := arm64-micro-characterization-image-tar
MICRO_IMAGE.x86.full := $(X86_RUNNER_RUNTIME_IMAGE)
MICRO_IMAGE.arm64.full := $(ARM64_RUNNER_RUNTIME_IMAGE)
MICRO_IMAGE.x86.characterization := $(X86_MICRO_CHARACTERIZATION_IMAGE)
MICRO_IMAGE.arm64.characterization := $(ARM64_MICRO_CHARACTERIZATION_IMAGE)
micro-aws-x86: RUNTIME_CONTAINER_IMAGE.aws-x86 := $(MICRO_IMAGE.x86.$(MICRO_RUNTIME_PROFILE))
micro-aws-arm64: RUNTIME_CONTAINER_IMAGE.aws-arm64 := $(MICRO_IMAGE.arm64.$(MICRO_RUNTIME_PROFILE))
micro-aws-x86: $(MICRO_IMAGE_TARGET.x86.$(MICRO_RUNTIME_PROFILE))
micro-aws-arm64: $(MICRO_IMAGE_TARGET.arm64.$(MICRO_RUNTIME_PROFILE))
corpus-aws-x86: x86-runner-runtime-image-tar
corpus-aws-arm64: arm64-runner-runtime-image-tar
